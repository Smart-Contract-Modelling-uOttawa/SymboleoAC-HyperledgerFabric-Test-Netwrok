...
*/
  ...
  return {successful: true}
   ...
   /**
   * Stores the hardcoded rolesList in the ledger as ACPolicyRecord with a hash.
   * Can only be called by Regulator. Input is not accepted to prevent tampering.
   */
  /*
  async storeRolesPolicy(ctx, contractId) {
    ...
    console.log(contract._roles)
    const policy = {// Build roles policy from hardcoded list in contract spec
      roles: contract._roles.map(role => ({
         name: role._name,
         type: role._type,
         dept: role.dept._value,
    ...
    const record = {
      hash: policyHash,
      policy,
      verified: false // Can be updated if signature verification is added
    };

    await ctx.stub.putState('ACPolicyRecord', Buffer.from(JSON.stringify(record)));

    // Emit event for access audit
    await ctx.stub.setEvent('ACPolicyAccessed', Buffer.from(JSON.stringify({ accessor: userId, role: role, time: new Date().toISOString() })));

    return {
      successful: true,
      hash: policyHash,
      message: 'ACPolicy stored successfully'
    };
  }*/
  ...
  async getRolePolicy(ctx, contractId) {
    ...
    const policyBytes = await ctx.stub.getState('ACPolicyRecord');
    if (!policyBytes || policyBytes.length === 0) {
      //throw new Error('ACPolicyRecord not found');
      return { successful: false, message: 'ACPolicyRecord not found' };
    }
    ...
    //return policyBytes.toString(); // JSON: { hash, policy, metadata, ... }
  }*/
  ...
  async violateObligation_payment(ctx, contractId) {
    console.log("I am in violateObligation_payment")
    const contractState = await ctx.stub.getState(contractId)
    if (contractState == null) {
       console.log("false1")
      return {successful: false}
    ...
    const contract = deserialize(contractState.toString())
    this.initialize(contract)
    console.log("I am in violateObligation_payment after initlize contract")
