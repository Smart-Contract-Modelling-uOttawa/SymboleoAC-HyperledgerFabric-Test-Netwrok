     ...
     class HFContract extends Contract {
       ...
       async init(ctx, args) {
         ...
         const contractInstance = new VaccineProcurementC (inputs.pfizerP, inputs.mcdcP, inputs.regulatorP, inputs.adminP, inputs.fdaP, inputs.worldcourierP, inputs.approval, inputs.unitPrice, inputs.minQuantity, inputs.maxQuantity)
       ...
       async trigger_mcdcTerminateAgreement(ctx, args) {
       	const cid = new ClientIdentity(ctx.stub);
       	let roleObj;
       	const inputs = JSON.parse(args);
       	const contractId = inputs.contractId;
         ...
         if (contract.isInEffect()  ){
         	...
         	try{           	
         	    ...
         	    //seond layer
         	    let controllers = contract.mcdcTerminateAgreement._controller
         	if(!contract.accessPolicy.hasPermesstion('grant','read', contract.mcdcTerminateAgreement,roleObj, contract.mcdcTerminateAgreement.getController(controllers.length - 1)) || 
         	      !contract.accessPolicy.isValid(new Rule('grant','read', contract.mcdcTerminateAgreement, roleObj, contract.mcdcTerminateAgreement.getController(controllers.length - 1))) ){
         	        throw new Error(`access denied...`)
         	      }
           contract.mcdcTerminateAgreement.happen(event)
           Events.emitEvent(contract, new InternalEvent(InternalEventSource.contractEvent, InternalEventType.contractEvent.Happened, contract.mcdcTerminateAgreement))
            //notification
            for (const message of contract.notified.message) {
            if (!oldMessagesList[0].includes(message)) {
                    this.trigger_notification(ctx, message)
           ...
           async getIoTCondition(ctx, contractId) {
             ...
             for (const key of eventList) {
               ...
               rules.rules.push({
                 ...
                 chaincodeName: "vaccineprocurementc",
      ...
      // Return the states of the contract and its parts     
       async getState(ctx, contractId) {
         ...
         if (contract.mcdcTerminateAgreement._triggered) {
           output += `  Event "mcdcTerminateAgreement" happened at ${contract.mcdcTerminateAgreement._timestamp}\r\n`
         } else {
           output += `  Event "mcdcTerminateAgreement" has not happened\r\n`
         }
