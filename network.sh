...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "CA_IMAGETAG=$CA_IMAGETAG"
    infoln "COMPOSE_FILE_CA=$COMPOSE_FILE_CA"
    IMAGE_TAG=${CA_IMAGETAG} docker-compose -f $COMPOSE_FILE_CA up -d 2>&1
    infoln "CA_IMAGE_TAG=$IMAGE_TAG"
...
# genesis block is defined using the "OneOrgOrdererGenesis" profile at the bottom
